...
reproducible. The **`PulseDeck.exe` payload is intentionally not committed**
(it's gitignored as `*.exe`) — drop the freshly built + signed exe in here
right before packing.

## Files
- `AppxManifest.xml` — package manifest. **Identity** (`PapanikolaouFokion.PulseDeck`,
...
copy ..\dist\PulseDeck.exe .\PulseDeck.exe
